...
def run_tables_to_csc(
    ...
    *,
    ...
    from .tables import convert_store_tables_to_csc

    result = _as_command_error(
        lambda: convert_store_tables_to_csc(
            source,
            dest,
            tables=tables,
            layers=layers,
...
def run_list_tables(zarr: str | Path) -> dict[str, Any]:
    from .store import list_element_keys

    keys = list_element_keys(zarr, "tables")
    if not keys:
        raise WriterCommandError(f"No table elements found under {Path(zarr) / 'tables'}")
    return {"zarr": str(zarr), "table_keys": keys}
