"""The openjph-wasm HTJ2K backend: vendored WASM driven by Node.js workers.

This is the same WASM build the JS reader uses, which makes it the reference for
encode/decode agreement with the browser. Prefer the `backends` module over
calling this directly — it selects between this and the Node-free `imagecodecs`
backend.
"""
...
@lru_cache(maxsize=1)
def wasm_encode_available() -> bool:
    """Return whether the vendored OpenJPH WASM helper (and Node.js) is usable."""
    try:
        plane = np.zeros((4, 4), dtype=np.uint16)
        encode_htj2k_wasm(plane, reversible=True, quality=0.0)
    except Exception:
        return False
    return True


def encode_htj2k_wasm(
    volume: np.ndarray,
    *,
...
def decode_htj2k_wasm(codestream: bytes | bytearray) -> np.ndarray:
