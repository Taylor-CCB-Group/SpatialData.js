...
export async function createLoader(
  ...
  // Otherwise load.
  try {
    // OME-NGFF ALWAYS (not like in Avivator itself, or MDV)
    //@ts-expect-error - loadOmeZarr `source` string vs UrlOrFiles
    const res = await loadOmeZarr(urlOrFile, { type: 'multiscales' });
    // extract metadata into OME-XML-like form
    const metadata = {
      Pixels: {
        Channels: res.metadata.omero.channels.map((c: { label: string }) => ({
          Name: c.label,
          SamplesPerPixel: 1,
        })),
      },
...
function isGlobalOrXYDimension(name: string): name is 'x' | 'y' {
  ...
  return (
    normalizedName === 'x' ||
    normalizedName === 'y' ||
    GLOBAL_SLIDER_DIMENSION_FIELDS.includes(normalizedName)
  );
}
 ...
 */
export function tryParseOmeroHexColor(
 ...
 * @param renderingMode
 */
export function get3DExtension(colormap: boolean, renderingMode: RenderingMode): unknown {
