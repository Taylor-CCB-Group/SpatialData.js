  ...
  const channelsStore = createStore((set) => ({
      ...
      set((state: ChannelsState) => {
        ...
        for (const key of Object.keys(state)) {
          if (channelKeys.includes(key)) {
            newState[key as keyof ChannelsState] = (
              state[key as keyof ChannelsState] as unknown[]
            ).filter((_, j) => j !== channel);
          }
        }
        return { ...state, ...newState };
      }),
    addChannel: (newProperties: NewChannelValues) =>
      set((state: ChannelsState) => {
        ...
        const newState: Partial<ChannelsState> = {};
        for (const [property, value] of entries) {
          newState[property as keyof ChannelsState] = [
            ...state[property as keyof ChannelsState],
            value,
          ];
        }
        for (const [k, v] of Object.entries(DEFAUlT_CHANNEL_VALUES)) {
          if (
            (newState[k as keyof ChannelsState] as unknown[]).length <
            (newState[entries[0][0] as keyof ChannelsState] as unknown[]).length
          ) {
