...
/**
 * A `PointsLayer` `tileDebugSignature` prop: changes whenever the store's tile
 * entries do, which is what makes the debug overlay redraw. The store itself is
 * identity-stable per layer, so without this the overlay's `updateTriggers` would
 * never fire.
 */
export function tileDebugSignature(store: TileDebugStore | undefined): string {
