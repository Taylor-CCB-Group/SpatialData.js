 ...
 */
export const v2ZarraySchema = v2ZarrayBaseSchema
  .refine((data: V2ZarrayInput) => data.chunks.length === data.shape.length, {
    message: 'chunks length must match shape length',
    path: ['chunks'],
  })
  .refine(
    (data: V2ZarrayInput) =>
      !data.dimension_names || data.dimension_names.length === data.shape.length,
    {
 ...
 */
export const v3ZarraySchema = v3ZarrayBaseSchema
  .refine(
 ...
 */
export function validateAndConvertV2Zarray(zarray: unknown, path: string): ZarrV3ArrayNode {
 ...
 * @throws Error if required fields are missing or invalid
 */
export function validateV3Zarray(zarray: unknown, path: string): ZarrV3ArrayNode {
