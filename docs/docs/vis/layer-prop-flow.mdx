...
- **Table-driven fill colours** should be computed from
  `@spatialdata/core`-resolved `rowIndexByFeatureIndex` and the shared
...
- precomputed per-feature colour bytes belong in `featureColorResolver` — a
  `Uint8Array` in `entry.props` would be a lie about what a saved config is
...
- **`buildShapeFillColorByFeatureId`**: converts a table column plus resolved
  row alignment into per-feature colours. It is renderer-agnostic and does not
  load tables or decide SpatialData feature/table association semantics.
...
Labels reuse the primitive above, with the one substitution their data forces. A
shape's colour texture is indexed by **feature index**; a label has no geometry —
it *is* a raster pixel value — so its table is indexed by the **label's own
integer instance id**, and the bitmask fragment shader samples the instance-id
raster and looks the integer up.

What carries over is the property that matters: a feature-state change re-uploads
only the small table, **never the tiles**.

- **The LUT texture is owned by `LabelsLayer`** and shared across tile sublayers.
  Per-tile textures would multiply a table that is already megabytes for a large
  segmentation.
- **Picking consults the same table**, so a hidden label can never be picked.
- **Label `0` is background** and never drawn, but is still filled in the table,
  so the shader reads it without a bounds branch per fragment.
- `LabelsResolver.fillColors` is keyed by element **and column** — keying by
...
`highlightedLabelId` uniform.

A uniform, not a bit in the LUT, is the whole point. The pointer moves constantly; baking
hover into the table would re-upload a texture that is megabytes for a large segmentation
on every move — exactly what the section above exists to prevent. Tiles are untouched
either way.
...
- **The tint prop is `labelHighlightColor`.** See the anti-patterns below.
...
| Naming a layer prop `highlightColor` | deck's own `Layer` owns that name and defaults it to navy `[0, 0, 128, 128]`, so your prop is never absent and your default can never apply — this silently drew every labels hover in deck's navy. Labels use `labelHighlightColor` |
