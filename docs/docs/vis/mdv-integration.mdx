...
See [Headless viewer guide](./headless-viewer) for experimentation steps. The `/headless` demo (`packages/vis/demo`) includes brightness/contrast sliders exercising `vivImagePropsResolver`.

## Issue #56 channel and extension APIs

Tracked in [GitHub issue #56](https://github.com/Taylor-CCB-Group/SpatialData.js/issues/56). These APIs let MDV replace its local MobX↔Zustand channel bridge.
...
- `ImageChannelPanel` — controlled export: `channels`, `onChannelsChange`, `defaults`.
...
Merge order into `detailView.getLayers({ props })`: saved `vivLayerProps` → `vivImagePropsResolver` → `extensions` from `vivImageExtensionResolver` (or global `vivImageExtensions`). `VivSpatialViewer` must not spread `layer.props` afterward.
