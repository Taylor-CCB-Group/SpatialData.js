...
- [ ] Track upgrading to `@hms-dbmi/viv@0.21.0` / deck.gl `9.2.9` as a first-class migration. Viv PR [hms-dbmi/viv#924](https://github.com/hms-dbmi/viv/pull/924) is merged there (uniform-buffer-backed shader props, `model.shaderInputs`, and variable channel counts).
...
MDV can compute this state from its datastore and filters, while `@spatialdata/layers` now owns the deck-facing shapes implementation and `@spatialdata/vis` consumes that shared path.
...
- shapes currently expose a render-oriented core payload with stable feature ids and a polygon fallback path; the shipping backend still uses decoded polygons, but the deck-facing implementation now lives in `@spatialdata/layers`.
...
- [ ] Make `@spatialdata/layers` responsible for choosing the rendering backend:
  ...
  - `deck.gl-geoarrow` as the intended near-term fast path when the core payload supports it cleanly
...
- **Viv/deck migration (Viv `0.21.0` / PR `#924`):** Merged upstream in `@hms-dbmi/viv@0.21.0` with deck.gl `9.2.9`; shader props use UBOs and `model.shaderInputs`. This repo and MDV still need to adopt that stack and migrate custom shader extensions deliberately, not piecemeal.
