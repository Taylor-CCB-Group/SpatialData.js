...
with a custom codec worker that registers JP2K support inside the worker before
decode. It loads `/test-fixtures/codecs/jpeg2k.zarr` and renders through the
store-backed OME-Zarr path in `SpatialCanvasViewer`. The manifest is available
at `/test-fixtures/codecs/jpeg2k.manifest.json`.
...
- `enableWorkerChunkDecode()` from `zarrextra/workers` for browser apps (fizarrita
  worker pool + custom codec worker with JP2K registration).
