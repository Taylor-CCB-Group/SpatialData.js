...
Note that as of now it only works with older stores (this should change reasonably soon).
...
5. **Eager Loading of Metadata**: Consolidated metadata from the zarr store is loaded up-front and used to build a representation of the object structure such that it can be programmatically exposed without `async` leaking where it isn't needed.
